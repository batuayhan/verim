 ...
 *
 * İki repoda senkron tutulur: apps/client/src/types ↔ apps/server/src/contract
 */
...
export interface ObjectSetLoadRequest {
  ...
  limit?: number;
  /**
   * totalCount için pahalı count(*) sorgusunu koş (varsayılan true — genel /objectsets
   * sözleşmesi korunur). İç çağrılar (reasoning/senkron) totalCount kullanmaz →
   * `false` geçince SQL motoru count sorgusunu ATLAR (37M satırlık seq-scan silinir).
   */
